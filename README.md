...
LINQ. [Scope and future work](#scope-and-future-work) records what a production version would add — and,
more usefully, the **threshold** at which each one starts paying for itself.
...
glyphs during play — which allocates. That decision has a known expiry date: the first
non-ASCII localisation needs a different font strategy.
...
|---|---|
| `Level_10x10` | The default. 10×10, six colours, eight Boxes, twenty moves — Example 1's colours and thresholds, on the largest board the case allows |
...
fires, because a board that size always has a pair somewhere — the measurement behind that is in
[Scope and future work](#scope-and-future-work). `Level_2x2` is the quickest way to see it.
...
| Global event bus | There are two parties and they already know each other |
| GPU instancing, one merged mesh | A single atlas already produces a single draw call |
...
| A `visitedStamp` instead of clearing | Clearing 100 ints is a `memset` |
...
**Explicit state machine** — a `GameState` enum with declared transitions
(`Idle → Resolving → Animating → Deadlock → Win/Lose`). There is one screen here, so `GameController`'s
implicit flow is enough; with a menu, a tutorial, booster selection, level intros and ad breaks, that
flow stops being followable through method calls.
**Threshold:** the second screen.
...
**Level content pipeline** — a level editor, level files, per-level spawner definitions. The case asks
for parametric generation (M, N, K, A, B, C all variable, every size from 2 to 10 playable), and
hand-authored levels pull against that.
**Threshold:** progression across levels.
...
*prevented* — the right trade here, because the case asks for detection and resolution by name, and a
preventive spawner would make that requirement invisible. A throwaway random-play harness put 2000
playthroughs through the default 10×10 and triggered the shuffle zero times: at that size there is no
deadlock problem for a weighted spawner to solve, only a difficulty curve for it to shape.
**Threshold:** when the difficulty curve becomes a product requirement.

**Automated level validation** — bots playing each level thousands of times to prove it is solvable.
Here the generation side is reduced to a single constraint ("no Box on the top row"), and the
validation side used the throwaway bot above: both playability bugs found during development came out
of measurement rather than reasoning. That bot is not in the repository, because the properties it
measured are tests now.
**Threshold:** hand-authored levels, or more than one obstacle type.

**Addressables and platform asset management** — compression profiles, memory budgets, lazy loading.
Meaningless for 26 sprites on one atlas.
**Threshold:** hundreds of assets, or multiple themes.
...
- **`ProfilerMarker` on the hot paths** (`Board.TryBlast`, `RecalculateGroups`), so the performance
  claim is legible in the Profiler rather than argued. Half an hour.
- **A toggleable debug overlay** — group ids, group sizes, deadlock state, which blocks are settled.
  Two hours, and it is the quickest way to show the deadlock shuffle actually firing.
- **More `[Conditional]` invariants.** Started already, where the caller-owned-workspace pattern left
  its one gap: `GroupFinder` and `GravityResolver` know their dimensions from the constructor but
  receive the data separately, so the two could drift. Remaining candidates: group sizes summing to
  the cell count, Box health staying inside 0–2. These vanish entirely from a release build.
- **A custom `LevelConfig` inspector** with board gizmos. Optional, and last of the four.

### Deliberately absent from the game itself

Boosters, special blocks, chained combo scoring, progression, save/load, cloud save, localisation.
None of them is asked for, and each one is scope spent away from what is.

The one exception worth naming is accessibility: the case's own rule that every colour carries a
different icon already does most of the work for colour-blind players, which is why no separate
accessibility mode is missing here.

---

## Further reading

`ARCHITECTURE.md` — the rules the code holds itself to, the data model, and the algorithms worth
explaining.
